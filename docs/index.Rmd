...
items %>%
    left_join(select(multilex_data$pool, te, language)) %>% 
    unnest(list) %>% 
...
```{r items-frequency, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE}
...
```{r items-nphon, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE}
...
```{r items-lv, echo=FALSE, message=FALSE, warning=FALSE, paged.print=TRUE}
...
```{r items-frequency-phonemes, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r items-frequency-levenshtein, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r items-phonemes-lv, message=FALSE, warning=FALSE, paged.print=FALSE}
items %>% 
    left_join(select(pool, item, language)) %>% 
    drop_na(freq) %>% 
...
participants %>% 
    mutate(dominant_language = ifelse(doe_catalan >= doe_spanish, "Catalan", "Spanish")) %>% 
...
```{r participants-age, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
participants %>% 
...
```{r participants-lp, message=FALSE, warning=FALSE, paged.print=FALSE}

participants %>% 
    mutate(
        dominant_language = ifelse(doe_catalan >= doe_spanish, "Catalan", "Spanish"),
        age = round(age),
        id = paste(id, "(", time, ")"),
    ...
    geom_col(width = 0.75, position = position_fill(), colour = "white", size = 0.1) +
    labs(
...
* `understands/produces ~ 1 + age_std + frequency + (1 + age_std | item) + (1 + frequency | id)`
* `understands/produces ~ 1 + age_std + frequency + doe + (1 + age_std + doe | item) + (1 + frequency | id)`
* `understands/produces ~ 1 + age_std + frequency + doe*cognate + (1 + age_std + doe | item) + (1 + frequency + cognate| id)`
...
```{r loos, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, eval = FALSE}
loo_comp %>% 
    as_tibble() %>% 
...
post <- gather_draws(model_fit_7, `b_.*`, regex = TRUE) %>% 
    mutate(
        ...
        .variable_name = factor(.variable, levels = names(var_repl), labels = var_repl)
    )
...
```{r random-participant, message=FALSE, warning=FALSE, paged.print=FALSE}

re_id <- ranef(model_fit_7)$id[,,1] %>% 
    as.data.frame() %>% 
    rownames_to_column("id") %>% 
    as_tibble() %>% 
    mutate_if(is.numeric, ~inv_logit_scaled(. + fixef(model_fit_7)[1])) %>% 
...
```{r re-item, message=FALSE, warning=FALSE, paged.print=FALSE}
