...
authors:
    ...
    - Daniela Avila-Varela^[Universitat Pompeu Fabra]
...
```{r items-frequency, echo=TRUE, message=FALSE, warning=FALSE, paged.print=TRUE}
...
```{r items-nphon, echo=TRUE, message=FALSE, warning=FALSE, paged.print=TRUE}
...
```{r items-lv, echo=TRUE, message=FALSE, warning=FALSE, paged.print=TRUE}
...
```{r participants-age, echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
participants %>% 
    mutate(age = floor(age)) %>% 
    count(age) %>% 
    ggplot() +
    aes(x = age, y = n) +
    geom_col(fill = pal_d3()(3)[3]) +
...
```{r participants_edu, echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
participants %>%
    mutate(
...
```{r loos, echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE, eval=FALSE}
model_loos %>% 
    as_tibble() %>% 
...
```{r marginal, message=FALSE, warning=FALSE, paged.print=FALSE}
nd <- expand.grid(
    n_phon_std = 0,
    freq_std = 0,
...
```{r diagnostics-traceplots, message=FALSE, warning=FALSE, paged.print=FALSE}
gather_draws(
    model_fit_7, 
    `b_.*`, `sd_te__.*`, `cor_te__.*`, 
    regex = TRUE, 
...
```{r diagnostics-rhat, message=FALSE, warning=FALSE, paged.print=FALSE}
...
```{r diagnostics-neff, message=FALSE, warning=FALSE, paged.print=FALSE}
