...
tar_load(posterior_description_re)
...
post_draws_list <-
    split(posterior_description, posterior_description$parameter)
```
...
posterior_description_re %>% 
    ...
    mutate(
        across(
            ...
            ~factor(
                .,
                levels = c("Intercept", "age_std", "exposure_std", "lv_std", "n_phon_std", "age_std:exposure_std", "age_std:lv_std", "exposure_std:lv_std", "age_std:exposure_std:lv_std"),
                labels = c("Intercept", "Age", "Exposure","Levenshtein", "Phonemes", "Age \u00d7 Exposure", "Age \u00d7 Levenshtein", "Exposure \u00d7 Levenshtein", "Age \u00d7 Exposure \u00d7 Levenshtein")
            )
        )
    ) %>%
    filter(var_1 != var_2) %>% 
    ggplot(aes(x = var_1, y = fct_rev(var_2), 
               colour = median, label = percent(median, accuracy = 0.1))) +
    geom_point(shape = 15, size = 20) +
    geom_text(colour = "black") +
    scale_colour_gradient2(low = clrs[1], 
                           mid = "grey", 
                           high = clrs[4],
                           midpoint = 0) +
...
posterior_description_re %>% 
    ...
    mutate(
        across(
            ...
            ~factor(
                .,
                levels = c("Intercept", "age_std", "exposure_std", "n_phon_std", "lv_std", "age_std:exposure_std", "age_std:lv_std"),
                labels = c("Intercept", "Age", "Exposure", "Phonemes", "Levenshtein", "Age \u00d7 Exposure", "Age \u00d7 Levenshtein")
            )
        )
    ) %>%
    filter(var_1 != var_2) %>% 
