...
options(
    ...
    ggplot2.ordinal.fill = ggsci::pal_d3()(4),
    ggplot2.ordinal.colour = ggsci::pal_d3()(4),
...
participants %>% 
    ...
    geom_line(size = 1, colour = pal_d3()(1)[1]) +
...
participants %>% 
    pivot_longer(c(doe_catalan, doe_spanish),
                 names_to = "language", values_to = "doe", 
                 names_transform = ~str_to_sentence(str_remove(., "doe_"))) %>% 
    mutate(doe = cut(doe, breaks = seq(0, 1, 0.1), include.lowest = TRUE),
           id = paste0(id, " (", time, ")")
    ) %>% 
    count(language, doe) %>% 
...
participants %>% 
    mutate(
        dominance = ifelse(doe_catalan >= doe_spanish,
                           "Dominant in Catalan",
                           "Dominant in Spanish"),
        id = paste0(id, " (", time, ")"),
        doe_other = 1 - (doe_spanish + doe_catalan)
    ) %>% 
    mutate(rank = rank(doe_catalan)) %>% 
    pivot_longer(
        starts_with("doe_"), 
        names_to = "language",
        values_to = "doe", 
    ...
    mutate(language = factor(language, levels = c("Catalan", "Spanish", "Other"))) %>%
    ggplot(aes(reorder(id, desc(rank)), doe, colour = language, fill = language)) +
    facet_wrap(~dominance, ncol = 1, scales = "free_x") +
    geom_line(aes(group = id), colour = "grey") +
    geom_point(size = 0.5) +
    labs(x = "Participant", y = "Degree of exposure (DoE)", 
...
items <- left_join(items, select(pool, te, item, language, class, semantic_category))
...
m %>% 
    ...
    scale_x_continuous(
        ...
        sec.axis = dup_axis(
            ...
            labels = months_to_years(
                rescale_variable(
                    (seq(10, 36, 4)-mean(df$age))/(sd(df$age)),
                    mean(df$age),
                    sd(df$age)
                )
            )
    ...
    scale_fill_brewer(palette = "Reds", labels = c("95%", "89%", "67%", "50%")) +
    labs(
        x = "Age (months)", 
...
get_loo_plot <- function(x){
    x$pointwise[,5] %>% 
        ...
        ggplot(aes(x, y)) +
        geom_point(
            size = 0.5, alpha = 0.1, colour = pal_d3()(3)[2], 
            shape = 1
        ) +
...
posterior_description %>% 
    ...
    select(parameter, median, ci_high, ci_low, rope_percentage, rhat, ess) %>% 
    gt(rowname_col = "parameter") %>% 
    fmt_number(c(1:4, 6:7)) %>% 
    fmt_percent(5) %>% 
...
post %>%
    ...
    annotate(geom = "rect", ymin = -Inf, ymax = Inf,
             ...
             colour = NA, alpha = 0.2, fill = pal_d3()(3)[2],) +
    geom_vline(xintercept = 0, size = 1, colour = pal_d3()(3)[2]) +
    stat_slab(
...
marg_effects %>% 
    ...
    geom_line(aes(group = drawid), alpha = 0.5, size = 0.75, colour = "grey") +
...
posterior_description_re %>% 
    ...
    filter(var_1 != var_2) %>% 
    ggplot(aes(x = var_1, y = fct_rev(var_2), colour = median, label = printnum(median))) +
...
posterior_description_re %>% 
    ...
    filter(var_1 != var_2) %>% 
    ggplot() +
    aes(x = var_1, y = fct_rev(var_2), colour = median, label = printnum(median)) +
...
mcmc_rhat_data(model_rhats$model_fit_4) %>% 
    ...
    geom_rug(alpha = 0.01, colour = pal_d3()(1)[1]) +
    labs(
        x = "R-hat",
        y = "Number of samples",
        colour = "Description",
