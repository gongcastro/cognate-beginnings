...
We initially fitted a base model (`model_fit_0`) that only included the predictors $Age$ (`age_std`), $Frequency$ (`freq_std`), $Phonemes$ (`n_phon_std`), and $DoE$ (`doe_std`) as nuisance parameters, along with random intercepts and slopes of all predictors by $Participant$ (`id`) and $Item$ (`te`), and all correlation parameters. We then expanded this model by adding the main effect of $Levenshtein$ (`lv_std`) (`model_fit_1`) and its random slope by $Participant$. Then, we added the $DoE \times Levenshtein$ (`doe_std:lv_std`) two-way interaction (`model_fit_2`) and its random slope by $Participant$. Finally, we added the $Age \times Levenshtein$ (`age_std:doe_std:lv_std`) three-way interaction with its random slope by $Participant$, along with a random slope for the $Age \times DoE$ (`age_std:doe_std`) two-way interaction.

### Model equation

```{mermaid}
//| label: fig-causal-diagram
//| fig-cap: Causal diagram
flowchart LR
  B(Age) --> A("P(Acquisition)")
  C(Exposure) --> A
  D(Frequency) --> A
  E(Phonemes) --> A
  F(Levenshtein) --> C
  F(Levenshtein) --> A
  A("P(Acquisition)") --> G[Understands/Says response]
```
...
$$\begin{aligned}
\nu_{ij} = & \beta_0 +\beta_1\mathrm{Age}_{i} + \beta_2 \mathrm{Freq}_{j} + \beta_3\mathrm{Phon}_{j} + \beta_4\mathrm{Exposure}_{ij} + \beta_5 \mathrm{Levenshtein}_{j} + & [\mbox{Main effects}]\\
&\beta_6\mathrm{(Age_{i} \cdot \mathrm{Exposure}_{ij}}) + \beta_7\mathrm{(\mathrm{Age}_{i} \cdot \mathrm{Levenshtein}_{j}}) +  \beta_8\mathrm{(Exposure_{ij} \cdot \mathrm{Levenshtein}_{j}}) + & [\mbox{Two-way interactions}]\\
&\beta_9{(\mathrm{Age}_{i} \cdot \mathrm{Exposure}_{ij} \cdot \mathrm{Levenshtein}_{j}}) + & [\mbox{Three-way interaction}]\\

&\theta_{0j}^{(\mathrm{TE})} + \theta_{1j}^{(\mathrm{TE})}\mathrm{Age}_{i} +  \theta_{2j}^{(\mathrm{TE})} \mathrm{Freq}_{j} + \theta_{3j}^{(\mathrm{TE})} \mathrm{Phon}_{j} + \theta_{4j}^{(\mathrm{TE})} \mathrm{Exposure}_{ij} +  \theta_{5j}^{(\mathrm{TE})} (\mathrm{Age}_{i} \cdot \mathrm{Exposure}_{ij}) + & [\mbox{Item-level effects}]\\

&\theta_{0j}^{(\mathrm{ID})} + \theta_{1j}^{(\mathrm{ID})}\mathrm{Age}_{i} +  \theta_{2j}^{(\mathrm{ID})} \mathrm{Freq}_{j} +   \theta_{3j}^{(\mathrm{ID})} \mathrm{Phon}_{j} + \theta_{4j}^{(\mathrm{ID})} \mathrm{Exposure}_{ij} + \theta_{5j}^{(\mathrm{ID})} \mathrm{Levenshtein}_{j} + \theta_{6j}^{(\mathrm{ID})} (\mathrm{Age}_{i} \cdot \mathrm{Exposure}_{ij}) & [\mbox{Participant-level effects}]
\end{aligned}$$
:::
...
m <- add_epred_draws(nd, model_fit_3_prior, ndraws = NULL, re_formula = NA) %>% 
    filter(.category != "No") %>% 
...
-   Iterations: `r format(dim(model_fit_7$fit)[1], big.mark = ",")` (`r format(dim(model_fit_7$fit)[1]/2, big.mark = ",")`)
...
```{r}
#| label: fig-results-raw
#| echo: true
#| message: false
#| warning: false
#| fig-width: 9
#| fig-height: 7
#| fig-cap: "Observed proportion of comprehension and production responses across ages. Colours indicate different values of phonological similarity (Levenshtein similarity, binned). Results for different degrees of exposure (binned) are plotted separately."
df %>%
    mutate(
        lv = cut_interval(lv*100, 5) %>% 
            gsub("([0-9.]+)", "\\1%", .) %>% 
            factor(levels = c("[0%,20%]", "(20%,40%]", "(40%,60%]", "(60%,80%]", "(80%,100%]")),
        doe = cut_interval(doe*100, 3) %>% 
            gsub("([0-9.]+)", "\\1%", .) %>% 
            factor(levels = c("[0%,33.3%]", "(33.3%,66.7%]", "(66.7%,100%]")) %>% 
            paste0("Degree of Exposure: ", .) %>% 
            factor(
                levels = paste0(
                    "Degree of Exposure: ", 
                    c("[0%,33.3%]", "(33.3%,66.7%]", "(66.7%,100%]")
                )
            ),
        age = floor(age),
        Understands = response %in% c("Understands", "Understands and Says"),
        `Understands and Says` = response %in% c("Understands and Says")
    ) %>% 
    group_by(te, lv, age, doe) %>%
    summarise(
        n = n(),
    ...
    mutate(
        prop_se = sd(prop)/sqrt(n),
        prop_lower = prop-prop_se,
        prop_upper = prop+prop_se,
        prop_lower = ifelse(prop_lower < 0, 0, prop_lower),
        prop_upper = ifelse(prop_upper >1, 1, prop_upper)
    ) %>% 
    ggplot() +
    aes(age, prop, colour = lv, fill = lv, shape = lv) +
    facet_grid(.category~doe) +
    geom_hline(yintercept = 0.5, colour = "grey", size = 0.5) +
    ...
    labs(
        ...
        shape = "Phonological similarity (Levenshtein)",
        linetype = "Phonological similarity (Levenshtein)"
    ) +
    scale_color_brewer(palette = "Reds") +
    # scale_colour_manual(values = wesanderson::wes_palette("Zissou1", n = 5, type = "continuous")[1:5]) +
...
#| warning: false
#| eval: false
#  loo(log_lik(model_fit_3, ndraws = 750, re_formula = NA))
```
...
#| fig-width: 7
rope_coefs %>% 
    mutate(study = str_replace(study, "Braginsky, Yurovsky, Marchman and Frank", "Braginsky et al."),
           estimate = abs(estimate)) %>% 
    ggplot(aes(estimate, y = study)) +
    geom_vline(xintercept = 0, size = 0.75, colour = "grey") +
    geom_vline(
        xintercept = median(rope_coefs$estimate),
        size = 1,
        colour = pal_d3()(4)[4]
    ) +
    stat_dots(
    ...
    annotate(
        geom = "text",
        label = paste0(
            "Grand median of cross-study
             smallest effect size of interest\n",
            "Median = ", round(median(abs(rope_coefs$estimate)), 3)
        ),
        x = median(abs(rope_coefs$estimate)-0.05),
        y = 4,
        hjust = 1,
        vjust = 1.2,
        size = 3.5,
    ...
    labs(
        x = "Absolute standard regression coefficient (logit scale)\n(Excluding intercepts and predictor age)",
        y = "Study"
    ) +
...
#| message: false
#| warning: false
post_draws <- gather_draws(model_fit_3, `b_.*`, regex = TRUE)
...
**Lexical frequency** had a small, positive, but inconclusive on the probability of acquisition substantially, ($\beta$ = `r percent(post_draws_list[["b_freq_std"]]$median, accuracy = 0.01)`, 95% *HDI* = [`r percent(post_draws_list[["b_freq_std"]]$ci_low, accuracy = 0.01)`, `r percent(post_draws_list[["b_freq_std"]]$ci_high, accuracy = 0.01)`]): `r percent(post_draws_list[["b_freq_std"]]$rope_percentage, accuracy = 0.01)` of its posterior samples overlapped with the *ROPE.*
...
m <- add_epred_draws(nd, model_fit_3, ndraws = NULL, re_formula = NA) %>% 
    mutate(
...
re_age <- ranef(model_fit_3)$id[,,"age_std"] 
...
```{r}
#| label: fig-results-random-id-freq
#| echo: true
#| message: false
#| warning: false
#| fig-width: 7
#| fig-cap: "Posterior random slope of lexical frequency by participant"
re_freq <- ranef(model_fit_3)$id[,,"freq_std"] 
...
re_n_phon <- ranef(model_fit_3)$id[,,"n_phon_std"] 
...
```{r}
#| label: fig-results-random-id-doe
#| echo: true
#| message: false
#| warning: false
#| fig-width: 7
#| fig-cap: "Posterior random slope of age by language degree of exposure"
# re_doe <- ranef(model_fit_3)$id[,,"doe_std"] 
# 
# re_doe %>% 
#     as_tibble() %>% 
#     clean_names() %>% 
...
# re_lv <- ranef(model_fit_3)$id[,,"lv_std"]
#
# re_lv %>%
#     as_tibble() %>%
#     clean_names() %>%
...
```{r}
#| label: fig-results-random-id-doe-lv
#| echo: true
#| message: false
#| warning: false
#| fig-width: 7
#| fig-cap: "Posterior random slope of age by participant"
# re_doe_lv <- ranef(model_fit_3)$id[,,"doe_std_lv_std"]
# 
# re_lv %>%
#     as_tibble() %>%
#     clean_names() %>%
...
re_age_doe <- ranef(model_fit_3)$id[,,"age_std:doe_std"]
...
re_age_lv <- ranef(model_fit_3)$id[,,"age_std:lv_std"]
...
```{r}
#| label: fig-results-random-id-age-doe-lv
#| echo: true
#| message: false
#| warning: false
#| fig-width: 7
#| fig-cap: "Posterior random slope of the age-by-language degree of exposure-by-phonological similarity interaction by participant"
re_age_doe_lv <- ranef(model_fit_3)$id[,,"age_std:doe_std:lv_std"]
...
posterior_description_re %>% 
    ...
    mutate(
        across(
            ...
            ~factor(
                .,
                levels = c("Intercept", "age_std", "freq_std", "n_phon_std", "doe_std", "lv_std", "age_std:doe_std", "age_std:lv_std"),
                labels = c("Intercept", "Age", "Frequency", "Phonemes", "DoE", "Levenshtein", "Age \u00d7 DoE", "Age \u00d7 Levenshtein")
            )
        )
    ) %>%
    filter(var_1 != var_2) %>% 
...
re_te <- ranef(model_fit_3)$te[,,"Intercept"] %>% 
    as_tibble() %>% 
...
re_age <- ranef(model_fit_3)$te[,,"age_std"]
...
```{r}
#| label: fig-results-random-te-freq
#| echo: true
#| message: false
#| warning: false
#| fig-width: 7
#| fig-cap: "Posterior random slope of lexical frequency by translation equivalent"
re_freq <- ranef(model_fit_3)$te[,,"freq_std"]
...
```{r}
#| label: fig-results-random-te-doe
#| echo: true
#| message: false
#| warning: false
#| fig-width: 7
#| fig-cap: "Posterior random slope of age by participant"
# re_doe <- ranef(model_fit_3)$te[,,"doe_std"]
# 
# re_doe %>% 
#     as_tibble() %>% 
#     clean_names() %>% 
...
```{r}
#| label: fig-results-random-te-age-doe
#| echo: true
#| message: false
#| warning: false
#| fig-width: 7
#| fig-cap: "Posterior random slope of age by participant"
re_age_doe <- ranef(model_fit_3)$te[,,"age_std:doe_std"]
...
#| fig-width: 7
#| eval: false
mcmc_rhat_data(model_rhats$model_fit_3) %>% 
    ggplot() +
    aes(x = value) +
    geom_histogram(
        bins = 100, 
...
#| fig-width: 7
#| eval: false
mcmc_neff_data(model_neffs$model_fit_3) %>% 
    ggplot() +
