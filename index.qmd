...
authors:
  ...
  - name: Ignacio Castillejo
    ...
    affiliations:
      - name: Universidad Atónoma de Madrid
...
#| label: setup
#| include: false
tar_load_globals() # load packages and options
tar_load_everything() # load targets as R objects
...
Maximum educational attainment of the participant parent(s).
...
items <- left_join(items, select(bvqdev::pool, te, item, language, class, semantic_category))

items %>%
    unnest(list) %>% 
    group_by(language, list) %>% 
    summarise_at(
        vars(freq_zipf, n_phon, lv), 
        list(
            n = ~sum(!is.na(.)),
...
Word lexical frequency ($Frequency$): lexical frequency of the word in its corresponding language, expressed as Zipf scores [@van2014subtlex; @zipf1949human]. This variable ranges from 0 to 7, and follows and approximates a normal distribution, with most values in a corpus ranging from 3 to 5 points. Lexical frequencies were extracted from the English corpora of the [CHILDES](https://childes.talkbank.org/) database using the [{childesr}](https://langcog.github.io/childes-db-website/) R package. Available words in the corpora were then mapped to their Catalan and Spanish translations. Responses to words with missing lexical frequency score were excluded from analyses of the total number of items). 
...
items %>% 
    ...
    stat_slab(slab_colour = "white", slab_size = 1, alpha = 0.5, scale = 0.5) +
    stat_dots(side = "left", scale = 0.5, position = position_nudge(x = -0.05)) + 
    stat_pointinterval(
...
items %>% 
    ...
    labs(x = "Levenshtein distance", y = "Probability density") +
    theme(
        panel.grid.major.x = element_blank(),
        legend.position = "top"
    )
...
items %>% 
    drop_na(freq_zipf) %>% 
    count(n_phon, freq_zipf, language) %>% 
    ggplot(aes(n_phon, freq_zipf, colour = language, fill = language)) +
    facet_wrap(~language) +
...
items %>% 
    left_join(select(bvqdev::pool, item, language)) %>% 
    drop_na(freq_zipf) %>% 
    count(freq_zipf, lv, language) %>% 
    ggplot(aes(freq_zipf, lv, colour = language, fill = language, size = n)) +
    facet_wrap(~language) +
    geom_point(size = 2, alpha = 0.25) +
    geom_smooth(method = "lm", formula = "y ~ x") +
    labs(
...
#| warning: false
#| fig-width: 7
#| fig-cap: "Association betwen number of phonemes and phonological similarity"
items %>% 
    left_join(select(bvqdev::pool, item, language)) %>% 
    drop_na(freq_zipf) %>% 
    count(n_phon, lv, language) %>% 
    ggplot(aes(n_phon, lv, colour = language, fill = language, size = n)) +
    facet_wrap(~language) +
...
### Language exposure-weighted lexical frequency

We created a predictor ($Exposure$) to account for the exposure rate of each child to each of the words their parents responded to, weighted by the child's exposure to the language the word belongs to. the exposure of the $i$-th child to the $j$-th word measure is the product of the lexical frequency of the word (Zipf score) and the child’s degree of exposure to the corresponding language (a proportion)
...
df %>% 
    mutate(
        ...
        exposure = cut(exposure, breaks = seq(0, 9, 1), include.lowest = TRUE)
    ) %>% 
    ...
    ggplot(aes(x = freq, y = exposure, colour = language, size = n)) +
    facet_wrap(~language) + 
    geom_point() +
    labs(
...
We extracted four variables of interest for each observation (describe in previous sections):

* Word lexical frequency ($Frequency$)
* Word length ($Phonemes$)
* Phonological similarity ($Levenshtein$)
* Participant age ($Age$)
* Word exposure index ($Exposure$)

We fitted a series of nested Bayesian ordinal multilevel models to sequentially test the effects of the independent variables (number of phonemes, exposure, age, and Levenshtein) over the three levels of word learning (no, production, comprehension). In all these models participants’ and items’ individual differences were controlled by estimating different parameters for each one (different intercepts and/or slopes). This is similar to the control conducted by ANCOVAs. Thus, independent variable effects account for collective differences in word learning (unexplained by participants’ or items’ individual differences). 

We initially fitted a base model (`model_fit_0`) that only included the predictors $Age$ (`age_std`), $Phonemes$ (`n_phon_std`), along with random intercepts and slopes of all predictors by $Participant$ (`id`) and $Item$ (`te`), and all correlation parameters. We then expanded this model by adding the main effect of $Exposure$ (`exposure`) (`model_fit_1`), and then $Levenshtein$ (`lv_std`) (`model_fit_2`) and their corresponding random slopes by $Participant$. Then, we added the $Exposure \times Levenshtein$ (`exposure_std:lv_std`) two-way interaction (`model_fit_3`) and its random slope by $Participant$. Finally, we added the $Age \times Exposure \times Levenshtein$ (`age_std:exposure_std:lv_std`) three-way interaction with its random slope by $Participant$, along with a random slope for the $Age \times Exposure$ (`age_std:exposure_std`) two-way interaction.

We compared the predictive performance of the models using Bayesian leave-one-out cross-validation (*LOO*), a relative fit index, via the `loo` R package. LOO consists in computing the average likelihood of each observation after estimating the model's parameters leaving that same observation out of the data set. Due to computational constraints, we performed LOO based on a random subsample of 750 samples (out of `r format(dim(model_fit_4$fit)[1], big.mark = ",")`) of the posterior distribution of the fixed effects of the model.
...
$$\begin{aligned}
\nu_{ij} = & \beta_0 +\beta_1\mathrm{Age}_{i} + \beta_2\mathrm{Phon}_{j} + \beta_3\mathrm{Exposure}_{ij} + \beta_4 \mathrm{Levenshtein}_{j} + & [\mbox{Main effects}]\\
&\beta_5\mathrm{(Age_{i} \cdot \mathrm{Exposure}_{ij}}) + \beta_6\mathrm{(\mathrm{Age}_{i} \cdot \mathrm{Levenshtein}_{j}}) +  \beta_7\mathrm{(Exposure_{ij} \cdot \mathrm{Levenshtein}_{j}}) + & [\mbox{Two-way interactions}]\\
&\beta_8{(\mathrm{Age}_{i} \cdot \mathrm{Exposure}_{ij} \cdot \mathrm{Levenshtein}_{j}}) + & [\mbox{Three-way interaction}]\\

&\theta_{0j}^{(\mathrm{TE})} + \theta_{1j}^{(\mathrm{TE})}\mathrm{Age}_{i} + \theta_{2j}^{(\mathrm{TE})} \mathrm{Phon}_{j} + \theta_{3j}^{(\mathrm{TE})} \mathrm{Exposure}_{ij} +  \theta_{4j}^{(\mathrm{TE})} (\mathrm{Age}_{i} \cdot \mathrm{Exposure}_{ij}) + & [\mbox{Item-level effects}]\\

&\theta_{0j}^{(\mathrm{ID})} + \theta_{1j}^{(\mathrm{ID})}\mathrm{Age}_{i} +   \theta_{2j}^{(\mathrm{ID})} \mathrm{Phon}_{j} + \theta_{3j}^{(\mathrm{ID})} \mathrm{Exposure}_{ij} + \theta_{4j}^{(\mathrm{ID})} \mathrm{Levenshtein}_{j} + \theta_{5j}^{(\mathrm{ID})} (\mathrm{Age}_{i} \cdot \mathrm{Exposure}_{ij}) & [\mbox{Participant-level effects}]
\end{aligned}$$
:::
...
In addition, we specified a $LKJ$ prior--$\sim LKJ(2)$ for the correlations between group-level coefficients.
...
get_loo_plot <- function(x){
    x$pointwise[,5] %>% 
        ...
        ggplot(aes(x, y)) +
        geom_point(
            size = 0.5, alpha = 0.1, colour = clrs[5], 
            shape = 1
        ) +
        ...
        scale_x_continuous(breaks = seq(0, length(x$pointwise[,5]), 50000)) +
        scale_y_continuous(
            limits = c(-0.6, 0.6)
        )
}
...
str_repl <- c(
    ...
    "b_exposure_std" = paste0("Exposure (+1 SD, ",  percent(sd(df$exposure)), ")"),
...
#| warning: false
#| fig-width: 7
intercept_footnote <- paste0(
    "Probability of comprehension or production at the grand age of the sample (", 
    round(mean(df$age), 2), " months)"
)

# summarise posterior draws
posterior_description %>% 
    mutate(parameter = factor(parameter, levels = names(str_repl), labels = str_repl)) %>%  
    select(parameter, median, ci_high, ci_low, rope_percentage) %>% 
    gt(rowname_col = "parameter") %>% 
    fmt_number(c(1:4), decimals = 2) %>% 
    fmt_percent(5, decimals = 2) %>% 
...
@fig-results-fixed shows the outputs of the model. **Age** was the predictor with the largest effect on the probability of acquisition ($\beta$ = `r printnum(post_draws_list[["b_age_std"]]$median, digits = 2)`, 95% *HDI* = [`r printnum(post_draws_list[["b_age_std"]]$ci_low, digits = 2)`, `r printnum(post_draws_list[["b_age_std"]]$ci_high, digits = 2)`]): at the steepest point of the acquisition trajectory of an average translation equivalent, the probability of acquisition increased `r percent(post_draws_list[["b_age_std"]]$median/(4*sd(df$age)), accuracy = 0.01)` every month.
...
post %>%
    ...
    annotate(geom = "rect", ymin = -Inf, ymax = Inf,
             xmin = rope_interval["lower"], xmax = rope_interval["upper"],
    ...
    stat_slab(
        ...
        size = 1, position = position_nudge(y = 0.15)
    ) +
    geom_errorbar(
        data = post_summary,
    ...
    geom_text(
        ...
        colour = "black", position = position_nudge(y = -0.3), size = 3
    ) +
    labs(
...
nd <- datagrid(
    ...
    n_phon_std = 0,
    lv_std = 0,
    exposure_std = seq_range(model_fit_4$data$exposure_std, 10),
    age_std = 0,
    id = NA,
    te = NA
)
...
marg_effects %>% 
    ...
    # geom_line(aes(group = drawid), alpha = 0.25, size = 0.75, colour = clrs[1]) +
    stat_summary(fun.data = median_hdi, geom = "ribbon",
                 colour = NA, alpha = 0.5, fill = clrs[1]) +
    stat_summary(fun = median, geom = "line", size = 1, color = clrs[1]) +
    labs(
        x = "Exposure index",
...
```{r}
#| label: fig-results-random-id-correlations
#| echo: true
#| message: false
#| warning: false
#| fig-width: 7
#| fig-cap: "Posterior correlations between random effects by participant"
posterior_description_re %>% 
    select(parameter, median) %>% 
    filter(str_detect(parameter, "cor_id__")) %>% 
    mutate(parameter = str_remove_all(parameter, "cor_id__")) %>%  
    separate(parameter, c("var_1", "var_2"), sep = "__") %>%
    mutate(
        across(
            c(var_1, var_2), 
            ~factor(
                .,
                levels = c("Intercept", "age_std", "n_phon_std", "exposure_std", "lv_std", "age_std:exposure_std", "age_std:lv_std", "exposure_std:lv_std", "age_std:exposure_std:lv_std"),
                labels = c("Intercept", "Age", "Phonemes", "Exposure", "Levenshtein", "Age \u00d7 Exposure", "Age \u00d7 Levenshtein", "Exposure \u00d7 Levenshtein", "Age \u00d7 Exposure \u00d7 Levenshtein")
            )
        )
    ) %>%
    filter(var_1 != var_2) %>% 
    ggplot(aes(x = var_1, y = fct_rev(var_2), colour = median, label = percent(median, accuracy = 0.1))) +
    geom_point(shape = 15, size = 20) +
    geom_text(colour = "black") +
    scale_colour_gradientn(colours = clrs) +
    labs(colour = "Correlation", size = "Correlation") +
    theme(
...
```{r}
#| label: fig-results-random-te-correlations
#| echo: true
#| message: false
#| warning: false
#| fig-width: 7
#| fig-cap: "Posterior correlations between random effects by translation equivalent"
posterior_description_re %>% 
    select(parameter, median) %>% 
    filter(str_detect(parameter, "cor_te__")) %>% 
    mutate(parameter = str_remove_all(parameter, "cor_te__")) %>%  
    separate(parameter, c("var_1", "var_2"), sep = "__") %>%
    mutate(
        across(
            c(var_1, var_2), 
            ~factor(
                .,
                levels = c("Intercept", "age_std", "n_phon_std", "exposure_std", "lv_std", "age_std:exposure_std", "age_std:lv_std"),
                labels = c("Intercept", "Age", "Phonemes", "Exposure", "Levenshtein", "Age \u00d7 Exposure", "Age \u00d7 Levenshtein")
            )
        )
    ) %>%
    filter(var_1 != var_2) %>% 
    ggplot() +
    aes(x = var_1, y = fct_rev(var_2), colour = median, label = percent(median, accuracy = 0.1)) +
    geom_point(shape = 15, size = 40) +
    geom_text(colour = "black") +
    scale_colour_gradientn(colours = clrs) +
    labs(
        colour = "Correlation",
...
```{r}
#| label: fig-simulation-based-calibration
#| fig-cap: "Posterior predictive checks. We simulated 500 datasets from our model by drawing random samples from its posterior distribution. We then computed the proportion of 'No', 'Understands', and 'Understands and Says' responses in each dataset. These simulated proportions are depicted in the plot as solid horizontal lines. Their variation indicate the uncertainy of the model about what proportion of each type of response would be observe in a newly collected dataset. We also plotted the proportion of each type of response in the observed dataset (with a ± 5% interval for comparison)."
predicted_dfs <- add_predicted_draws(
    object = model_fit_4, 
    newdata = df,
    re_formula = NULL,
    ndraws = 500,
...
predicted_dfs %>% 
    ...
    count(.draw, .prediction) %>%
    group_by(.draw) %>% 
    mutate(
        prop = n/sum(n),
        .prediction = factor(
            .prediction,
            levels = levels(df$response), 
            ordered = TRUE
        )
    ) %>% 
    ggplot(aes(.draw, prop, fill = .prediction, colour = .prediction)) +
    geom_line(aes(group = .prediction)) +
    geom_rect(
        data = observed_prop,
        aes(xmin = 0, xmax = max(predicted_dfs$.draw),
            ymin = prop-0.025, ymax = prop+0.025,
            colour = .prediction, fill = .prediction),
        alpha = 0.25, colour = NA, inherit.aes = FALSE
    ) +
    # geom_segment(
    #     data = observed_prop,
    #     aes(x = -Inf, xend = Inf, y = prop, yend = prop,
    #         colour = .prediction),
    ...
    scale_x_continuous(labels = label_number(big.mark = ","), 
                       breaks = seq(0, max(predicted_dfs$.draw), 100)) +
