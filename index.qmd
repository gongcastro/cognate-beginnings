...
items %>% 
    drop_na(freq_zipf) %>% 
    distinct(te, .keep_all = TRUE) %>% 
    count(n_phon, freq_zipf) %>% 
    ggplot(aes(n_phon, freq_zipf)) +
...
items %>% 
    ...
    drop_na(freq_zipf) %>% 
    count(freq_zipf, lv) %>% 
    ggplot(aes(freq_zipf, lv, size = n)) +
...
For instance, for a child who is reportedly exposure to Catalan 80% of the time, and to Spanish 20% of the time, the expected exposure to the word *cavall* (horse, in Catalan, with a lexical frequency of `r round(items$freq_zipf[items$item=="cat_cavall"], 2)`) would be `r round(items$freq_zipf[items$item=="cat_cavall"]*0.80, 2)`, while that of its translation to Spanish *caballo* would be `r round(items$freq_zipf[items$item=="cat_cavall"]*0.20, 2)` (see @eq-exposure-example):
...
```{tikz dag, engine.opts=font_opts, echo=FALSE}
#| fig-cap: "Directed acyclic graph indicating the hypothesised causal relationship between the variables of interest. *i* and *j* indicate the variables that are affected by individual differences at the level of participants and translation equivalents, respectively. Unobserved variables are encircled, and observed variables are not. We hypothesised that the number of phonemes of the word, the age of the participant, the amount of exposure to the word, and the Levenshtein distance all affect participants' familiarity with the word (i.e., the probability of acquisition), and therefore the probability with which each response category is generated. We further hypothesised that age influences the effect of word exposure and Levenshtein distance on familiarity, and that word exposure influences the effect of Levenshtein distance on familiarity."
#| fig-align: center
#| fig-ext: svg
#| out-width: 100%
...
\begin{tikzpicture}
% nodes
\node (age) at (0,0) {$\text{Age}_{i, j}$};
\node (phonemes) at (3,4) {$\text{Phonemes}_{i, j}$};
\node (exposure) at (3,2) {$\text{Exposure}_{i, j}$};
\node (levenshtein) at (3,-2) {$\text{Levenshtein}_{i}$};
\node [ellipse, draw](familiarity) at (7, 0) {$Familiarity$};
\node (response) at (11, 0) {$\text{Response}$};
% arrows
\draw [->, shorten >= 4pt, shorten <= 4pt] (age) -- (familiarity);
\draw [->, shorten >= 4pt, shorten <= 4pt] (age) -- (exposure);
\draw [->, shorten >= 4pt, shorten <= 4pt] (age) -- (levenshtein);
\draw [->, shorten >= 4pt, shorten <= 4pt] (exposure) -- (familiarity);
\draw [->, shorten >= 4pt, shorten <= 4pt] (exposure) -- (levenshtein);
\draw [->, shorten >= 4pt, shorten <= 4pt] (phonemes) -- (familiarity);
\draw [->, shorten >= 4pt, shorten <= 4pt] (levenshtein) -- (familiarity);
\draw [->, shorten >= 4pt, shorten <= 4pt] (familiarity) -- (response);
...
responses <- data.frame(
    ...
    xmin = -Inf,
    xmax = Inf,
    .category = str_wrap(c("No", "Understands", "Understands and Says"), width = 12)
)

responses %>%
    ggplot(aes(ymin = ymin, ymax = ymax, xmin = xmin, xmax = xmax, fill = .category)) +
    geom_rect(alpha = 0.5) +
    stat_dist_slab(
        data = dist_df, 
    ...
    geom_hline(yintercept = c(-1.5, 1.5), linetype = "dotted", size = 1) +
...
marg_effects %>% 
    ...
    geom_line(aes(group = drawid), alpha = 0.25, size = 0.75, colour = clrs[4]) +
    # stat_summary(fun.data = median_hdi, geom = "ribbon",
...
get_loo_plot <- function(x){
    x$pointwise[,5] %>% 
        ...
        ggplot(aes(x, y)) +
        geom_point(
            size = 0.5, alpha = 0.1, colour = clrs[4], 
            shape = 1
        ) +
