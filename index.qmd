...
ggsave("plot.png", height = 6, width = 9)
```
...
#| fig-width: 7
str_repl <- c(
    "Intercept\\[1\\]" = paste0("Intercept (Comprehension and Production at ",  round(mean(df$age, 2)), " months)"),
    "Intercept\\[2\\]" = paste0("Intercept (Comprehension at ", round(mean(df$age, 2)), " months)"),
    ...
    "age_std" = paste0("Age (+1 SD, ",  round(sd(df$age), 2), " months)"),
    "freq_std" = paste0("Frequency (+1 SD, ",  round(sd(df$freq), 2), " Zipf)"),
    "n_phon_std" = paste0("Phonemes (+1 SD, ",  round(sd(df$n_phon), 2), " phonemes)"),
    "doe_std" = paste0("DoE (+1 SD, ",  percent(sd(df$doe)), ")"),
    "lv_std" = paste0("Levenshtein (+1 SD, ",  percent(sd(df$lv)), ")")
)

intercept_footnote <- paste0(
    "Probability of comprehension or production at the average age of the sample (", round(mean(df$age), 2), " months), transformed to the probability scale using the inverse logit."
)

slopes_footnote <- "Transformed using the divide-by-four rule to get the maximum change in probability of correct response, associated with a unit increase in this variable."


draws_fix <- fixef(model_fit_3) %>% 
    as.data.frame() %>%
    rownames_to_column("term") %>% 
    clean_names() %>% 
    select(-est_error) %>% 
    mutate(
        estimate_p = ifelse(grepl("Intercept", term), plogis(estimate), estimate/4),
        q2_5_p = ifelse(grepl("Intercept", term), plogis(q2_5), q2_5/4),
        q97_5_p = ifelse(grepl("Intercept", term), plogis(q97_5), q97_5/4),
        term = str_replace_all(term, str_repl),
        group = ifelse(grepl("Intercept", term), "Intercepts", "Slopes")
    )

gt(draws_fix, rowname_col = "term") %>% 
    fmt_number(2:7, decimals = 2) %>% 
    cols_merge(c("q2_5", "q97_5"), pattern = "[{1}, {2}]") %>% 
    cols_merge(c("q2_5_p", "q97_5_p"), pattern = "[{1}, {2}]") %>% 
    tab_spanner("Logit scale", c(estimate, q2_5, q97_5)) %>% 
    tab_spanner("Probability scale", c(estimate_p, q2_5_p, q97_5_p)) %>% 
    tab_row_group(label = "Intercepts", rows = 1:2) %>% 
    tab_row_group(label = "Slopes", rows = 3:nrow(draws_fix)) %>% 
    row_group_order(groups = c("Intercepts", "Slopes")) %>% 
    cols_label(
        term = md("**Predictor**"),
        estimate = "Mean",
        q2_5 = md("95\\% CrI"),
    ...
    tab_footnote(
        footnote = intercept_footnote,
        locations = cells_body(
            columns = "term", 
            rows = grepl("Intercept", term)
        )
    ) %>% 
    tab_footnote(
        footnote = slopes_footnote,
        locations = cells_body(
            columns = "term", 
            rows = term %in% str_repl[-c(1, 2)]
        )
    ) %>% 
    cols_align(
        align = c("right"),
    ...
    tab_style(
        list(cell_text(style = "italic"), cell_fill(color = "grey")),
        cells_row_groups()
    ) %>% 
    cols_hide(group)
```
...
post <- gather_draws(model_fit_3, `b_.*`, regex = TRUE) %>% 
    mutate(
        .value = ifelse(
...
post %>%
    ...
    geom_errorbar(
        data = post %>% 
            filter(!grepl("Intercept|sd", .variable)) %>%
            mean_hdi(.exclude = c(".chain", ".iteration", ".draw", ".row", ".variable_name")) %>% 
            mutate(
                .variable_name = factor(
                    .variable, 
                    levels = names(str_repl),
    ...
    geom_text(
        ...
        aes(
            label = paste0(
                ...
                " [", percent(.lower, accuracy = 0.1), ", ",
                percent(.upper, accuracy = 0.1), "]"
            )
        ),
...
d <- df %>%
    mutate(
        ...
        age_std = ((floor(age)-mean(df$age))/(sd(df$age))) %>% 
            cut(
                .,
                breaks = ((seq(10, 38, 4)-mean(df$age))/(sd(df$age))),
                labels = ((seq(10, 34, 4)-mean(df$age))/(sd(df$age))),
                include.lowest = TRUE
            ) %>% 
            as.character() %>% 
            as.numeric(),
...
# group_by(age_std, doe_std, lv_std, .category) %>% 
# summarise(
...
# mutate(
#     .epred_se = .epred_sd/n,
#     .epred_lower = .epred_mean-.epred_se,
#     .epred_upper = .epred_mean+.epred_se,
#     .category = str_replace_all(
#         .category, 
#         c(
#             "Understands" = "Comprehension\n(Understands)",
...
#     )
# ) %>% 
# select(.category, age_std, doe_std, lv_std, .epred = .epred_mean, .epred_lower, .epred_upper, n)
...
m %>% 
    ...
    # geom_line(
    #     aes(group = interaction(.draw, lv_std, doe_std), colour = lv_std),
    #     alpha = 0.05, 
    #     size = 1
    # ) +
    # stat_lineribbon(size = 1, alpha = 0.5, .width = 0.95, colour = NA) +
    geom_point(
        data = d,
...
ggsave("plot.png", height = 9, width = 12)
```
