...
var_levels <- c("b_Intercept", "b_age", "b_frequency_center", "b_lp1", "b_cognate1", "b_cognate2", "b_lp1:cognate1", "b_lp1:cognate2")
var_labels <- c("Intercept", "Age (+1 month)", "Frequency (+1 SD)", "Lang. Profile (Bil. - Mon.)", "Cognateness 1 (IDC+NIC - NC)", "Cognateness 2 (IDC -NIC)", "Lang. Profile \u00d7 Cognateness 1", "Lang. Profile \u00d7 Cognateness 2")
...
ggplot(post, aes(.value, fct_rev(.variable))) +
  geom_vline(xintercept = 0, size = 0.75, colour = "grey") +
  stat_pointinterval(colour = "white") +
  labs(x = "Production posterior probability", y = "Variable", fill = "Variable") +
  scale_x_continuous(labels = percent) +
  theme_github() +
  theme(
    legend.position = "none",
...
re_id <- ranef(fits_comp$fit_2)$id[,,1] %>% 
  ...
  left_join(distinct(fits_comp$fit_2$data, id, lp))

ggplot(re_id, aes(Estimate, reorder(id, Estimate))) +
  geom_errorbarh(aes(xmin = Q2.5, xmax = Q97.5), size = 0.25, colour = "white", height = 0) +
...
re_te <- ranef(fits_comp$fit_2)$te[,,1] %>% 
  ...
  left_join(distinct(fits_comp$fit_2$data, te, lp))

ggplot(re_te, aes(Estimate, reorder(te, Estimate))) +
  geom_errorbarh(aes(xmin = Q2.5, xmax = Q97.5), size = 0.25, colour = "white", heigth = 0) +
  geom_point(size = 0.5, colour = "white") +
  geom_vline(xintercept = 0.5, colour = "white") +
...
re_id <- ranef(fits_prod$fit_2)$id[,,1] %>% 
  ...
  left_join(distinct(fits_prod$fit_2$data, id, lp))

ggplot(re_id, aes(Estimate, reorder(id, Estimate))) +
  geom_errorbarh(aes(xmin = Q2.5, xmax = Q97.5), size = 0.25, colour = "white", heigth = 0) +
  geom_point(size = 0.5, colour = "white") +
  geom_vline(xintercept = 0.5, colour = "white") +
  labs(x = "Item average probability of production",
       y = "Item") +
  scale_color_brewer(palette = "Set1") +
  scale_x_continuous(limits = c(0, 1)) +
  theme_github() +
...
re_te <- ranef(fits_prod$fit_2)$te[,,1] %>% 
  ...
  mutate_if(is.numeric, function(x) inv_logit_scaled(x + fixef(fits_prod$fit_2)[1])) %>% 
  mutate(te = as.numeric(te)) %>% 
  left_join(distinct(fits_prod$fit_2$data, te, lp))
...
m <- add_fitted_draws(nd, fits_comp$fit_2, re_formula = NA, n = 25) %>% 
...
m <- add_fitted_draws(nd, fits_prod$fit_2, re_formula = NA, n = 25) %>% 
  mutate(frequency_center = as.factor(paste0(frequency_center, " SD")))
