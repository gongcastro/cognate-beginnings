...
summary(fits_comp$fit_2)$fixed %>% 
  ...
  tab_footnote(
    ...
    locations = cells_body(columns = "term", rows = term %in% c("age_center", "frequency", "doe_center", "cognate1", "doe_center:cognate1"))
  ) %>% 
  tab_footnote(
...
ggplot(m, aes(age_center, .value, colour = cognate, fill = cognate)) +
