...
When compared to their monolingual peers, they know fewer words only when just on language is considered (e.g., English monolinguals know more words in English than English-Spanish bilinguals). When both languages are taken into account, bilingual children  seem to know, at least, as many words as monolinguals do [@ben-zeev_influence_1977; @doyle_some_1977; @rosenblum_word_1983; @fernandez_bilingual_1992; @pearson_patterns_1994; @bialystok_receptive_2010; @hoff_dual_2012; @core_total_2013; @hoff_expressive_2014; @blom_cross-language_2019; but see @pearson_lexical_1993; @houwer_bilingualmonolingual_2014].
...
```{r results}
fit_9 <- readRDS(here("Results", "fit_9.rds"))
...
as.data.frame(loos$diffs) %>% 
  ...
  select(model, elpd_loo, se_elpd_loo, p_loo, elpd_diff, se_diff) %>% 
  gt() %>% 
  tab_spanner(
    label = md("**Comparison**"),
...
cbind(as.data.frame(exp(fixef(fit_9))),
      as.data.frame(inv_logit_scaled(fixef(fit_9)))) %>% 
  rownames_to_column("coefficient") %>% 
  mutate(coefficient = c(
    "Intercept 1 (Understands)",
    "Intercept 2 (Understands & Says)",
    "Age",
    "Frequency",
    "Dominance",
    "Bilingualism",
  ...
  clean_names() %>% 
  gt() %>% 
  tab_spanner(label = md("**Odds Ratio**"), columns = 2:5) %>% 
  tab_spanner(label = md("**Probability**"), columns = 6:8) %>% 
  fmt_number(2:9) %>% 
  cols_merge_range(col_begin = "q2_5", col_end = "q97_5") %>% 
  cols_merge_range(col_begin = "q2_5_1", col_end = "q97_5_1") %>% 
  cols_label(
  ...
  tab_footnote(
    ...
    locations = cells_column_labels(columns = vars("est_error"))
  ) %>% 
  tab_footnote(
    footnote = "95% CrI: 95% credible interval",
    locations = cells_column_labels(columns = vars("q2_5"))
  )
...
post_preds <- add_fitted_draws(nd, fit_9, n = 20) %>% 
  mutate(doe = doe %>%
           as.character() %>%  
           str_replace_all(c("0" = "Mean","1" = "1 SD")) %>% 
           paste0("DOE = ", .) %>% 
           factor(levels = c("DOE = -1 SD", "DOE = Mean","DOE = 1 SD"), ordered = TRUE)) %>% 
  mutate(
    .category = case_when(
      .category==1 ~ "None",
...
The age at which the average item changed 
