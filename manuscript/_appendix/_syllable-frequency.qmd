...
syll |> 
    group_by(language) |> 
    top_n(20, freq_syll) |> 
    ggplot(aes(tidytext::reorder_within(syll, desc(freq_syll), language),
               freq_syll)) +
    facet_wrap(~ language,
               scales = "free_x",
               ncol = 1) +
...
str_repl <- c(
    ...
    "b_n_syll_std" = glue::glue("Syllables (+1 SD, {round(sd(syllables_data$n_syll), 3)})"),
    "b_lv_std" = glue::glue("Cognateness (+1 SD, {round(sd(syllables_data$lv), 2)})")
)

post_syllables <- model_fit_syllables |>
    bayestestR::describe_posterior(test = "rope",
    ...
    mutate(.variable_name = factor(
        ...
        labels = str_repl) |>
            as.character(),
        parameter = ifelse(grepl("Intercept", parameter),
                           stringr::str_remove_all(parameter, "Intercept \\(|\\)"),
                           parameter)) |>
