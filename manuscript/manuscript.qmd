...
We manually generated a broad phonological transcription of every word included in the vocabulary checklists in X-SAMPA format [@wells1995computercoding]. Catalan word-forms were transcribed to Central Catalan phonology, and Spanish word-forms were transcribed to Castilian Spanish phonology. For each translation equivalent, we calculated the phonological similarity between the Catalan and the Spanish word-form using the Levenshtein distance, which measures the edit distance between each pair of phoneme strings [@levenshtein1966binary]. The output indicates the number of editions (insertions, deletions, or substitutions) that one string of phonemes/characters must go through to become identical to the other. We then divided this value by the length of the longest phoneme string and subtracted the outcome from one so that it could be interpreted in terms of phonological similarity, instead of phonological distance. This led to a distance metric that ranged from zero to one, where zero indicates that both word-forms are completely different, and one indicates that the two word-forms are identical [see @floccia2018introduction; @heeringa2003norwegian; @schepens2012distributions; @fourtassi2020growth; @laing2022phonological for similar approaches].
...
We modelled the probability of participants answering each response category (*No* < *Understands* < *Understands and Says*) using a Bayesian, multilevel, ordinal regression model. This model allowed us to estimate both item and participant word-acquisition trajectories, while estimating the effect of our variables of interest. (1) *Age*, calculated as the number of months elapsed between participants' birth date and questionnaire completion). (2) *Length*, the number of phonemes in the X-SAMPA phonological transcription of the word-form. (3) *Exposure*, a language exposure-weighted lexical frequency. To account for the fact that bilinguals' exposure to a given word-form is a function of the word-forms lexical frequency, but also of the quantitative input they receive from the language such word-form belongs to, we expressed lexical frequencies as the product between both variables. First, we extracted the child-directed lexical frequency of each word from the CHILDES database [@macwhinney2000childes; @sanchez2019childesdb]. Using the corresponding lexical frequencies directly from Catalan and Spanish was not possible due to the low number of Catalan participants and tokens available in their corresponding CHILDES corpora, so they were extracted from the English corpora instead. We then mapped the lexical frequencies of the English words to their Catalan and Spanish translations [see @fourtassi2020growth for a similar approach], and transformed them to Zipf scores [@vanheuven2014subtlexuk; @zipf1949human]. We then multiplied the resulting lexical frequencies by the reported degree of exposure of the child to Catalan or Spanish. For instance, for a child who is reportedly exposure to Catalan 80% of the time, and to Spanish 20% of the time, the expected exposure score to the Catalan word *cavall* [*horse*]---with a lexical frequency of `r round(items$freq[items$item=="cat_cavall"], 2)`---would be `r round(items$freq[items$item=="cat_cavall"]*0.80, 2)`, while that of its translation to Spanish *caballo* would be `r round(items$freq[items$item=="cat_cavall"]*0.20, 2)`. (4) *Cognateness*, defined as the phonological similarity between translation equivalents, and calculated as detailed in the *Questionnaire* section.  These predictors were included as main effects in the model, together with their two-way and three-way interactions. Predictors were standardised before entering the model by subtracting the mean of the predictor from each value and dividing the result by the standard deviation of the predictor. Participant-level and item-level random intercepts and slopes were included where appropriate, according to the structure of the data (see @eq-linear) [@barr2013random]. We specified a weakly informative prior around the parameters of the model (see Appendix C for details). 
...
Data processing and visualisation was done in R [@rcoreteam2013language, version `r r_version`] using the Tidyverse family of packages [@wickham2019welcome]. We implemented the model using `brms` [@burkner2017brms], a R interface to the Stan probabilistic language (`r stan_version`) [@carpenter2017stan]. We ran four MCMC  chains with `r format(niterations(model_fit), big.mark = ",")` iterations each and an additional `r format(niterations(model_fit), big.mark = ",")` warm-up iterations per chain. Model posterior draws and predictions were handled using the `tidybayes` [@kay2021tidybayes] and `marginaleffects` [@arel-bundock2022marginaleffects] R packages.
...
#| out-width: 100%
#| eval: true
#| fig-cap: "Posterior marginal effects. Lines correspond to 100 posterior-predicted medians. Different colours indicate different levels of cognateness (phonological similarity). Black lines indicate the median of the posterior predictions. Predictions are presented separately for different degrees of word exposure index: little exposure to the word, mean exposure, and high exposure. Predictions for Comprehension are shown on top and predictions for Comprehension and Production are shown on the bottom. In-sample predictions lie inside the grey rectangles."
