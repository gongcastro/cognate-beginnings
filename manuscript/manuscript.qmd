  ...
  - name: Daniela S. Avila-Varela
    ...
    affiliations: 
      ...
      - id: ul
        ...
        department: Linguistics Center (CLUL), School of Arts and Humanities
...
format:
  arxiv-pdf:
    ...
    linenumbers: true
    doublespacing: false
    runninghead: "Cognate beginnings to bilingual lexical acquistion"
    include-in-header: 
      text: |
        \usepackage{tipa}
        \usepackage{booktabs}
...
tbl_data <- items |> 
    ...
    mutate(total = rowSums(across(a:d)),
           items = map(items, function(x) tail(x, 2))) |>  
    arrange(desc(total)) |> 
...
knitr::kable(tbl_data,
             ...
             col.names = col_names,
             align = "lccccl")  |> 
    kableExtra::row_spec(nrow(tbl_data)-1, 
                         hline_after = TRUE)
```
...
Data processing and visualisation was done in R [@rcoreteam2013language, version `r r_version`] using the Tidyverse family of packages [@wickham2019welcome]. We implemented the model using `brms` [@burkner2017brms], a R interface to the Stan probabilistic language (`r stan_version`) [@carpenter2017stan]. We ran four iteration chains with `r format(niterations(model_fit), big.mark = ",")` iterations each and an additional `r format(niterations(model_fit), big.mark = ",")` warm-up iterations per chain. Model posterior draws and predictions were handled using the `tidybayes` [@kay2021tidybayes] and `marginaleffects` [@arel-bundock2022marginaleffects] R packages.
...
tbl_data <- model_summary |>
    ...
    mutate(
        ...
        .hdi = paste0("[", round(.lower, 3),
                      ", ", round(.upper, 3), "]"),
        .rope_overlap = sub("^(-?)0.", "\\1.",
                            sprintf("%.3f", .rope_overlap))) |> 
    select(.variable_name, .median, .lower, .upper, .hdi, .rope_overlap) 
...
tbl_data |> 
    ...
    kableExtra::pack_rows("Intercepts (at 22 months)",
                          start_row = 1, 
                          end_row = 2) |> 
    kableExtra::pack_rows("Slopes (upper bound)",
                          start_row = 3,
                          end_row = 10) |> 
...
fig_data |> 
    ...
    scale_color_grey(start = 0.80, end = 0.1) +
    scale_fill_grey(start = 0.80, end = 0.1) +
