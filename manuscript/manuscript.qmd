...
re_data_prop <- re_data |>
    mutate(
        is_positive = .value > 0,
        is_negative = .value < 0
    ) |>
    summarise(across(matches("is_"), mean),
        .by = .variable_name
    ) |>
    mutate(prop = ifelse(is_positive > is_negative,
        is_positive,
        is_negative
    ))
```
