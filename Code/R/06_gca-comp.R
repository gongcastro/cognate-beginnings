#### 06_gca-comp: Bayesian non-linear model ##############
...
ggplot(posterior, aes(.value, .variable)) +
    ...
    ggsave(here("Figures", "06_gca_comp-coefs.png"))
...
ggplot(posterior, aes(.iteration, .value, colour = .chain)) +
    ...
    theme_ggdist() +
    theme(legend.position = "top") +
    ggsave(here("Figures", "07_gca_comp-convergence.png"))
...
posterior_check <- expand_grid(n = 1,
    ...
    mutate(cognate = factor(cognate, levels = c("Non-cognate", "Cognate"), ordered = TRUE),
           item_dominance = factor(bilingualism, levels = c(-1, 1), labels = c("Very bilingual", "Very monolingual")),
           .draw = as.factor(.draw))
