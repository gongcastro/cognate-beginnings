#### 05_analysis-comp: Bayesian non-linear model ##############
...
fit1_prior <- brm(formula = formula1,
                  ...
                  prior = priors1,
                  sample_prior = "only",
                  save_model = here("Code", "Stan", "comp_fit1-prior.stan"),
                  stan_model_args = list(save_dso = TRUE),
                  control = list(adapt_delta = 0.9, max_treedepth = 15))

#### compare models #########################
loo0 <- add_criterion(fit0, "loo", file = here("Results", "comp_fit0.rds"))
loo1 <- add_criterion(fit1, "loo", file = here("Results", "comp_fit1.rds"))
...
ggplot(posterior, aes(x = value, fill = model)) +
    ...
    theme_custom +
    ggsave(here("Figures", "07_analysis-comprehensive-posterior.png"), height = 6, width = 5.4)
...
posterior %>%
    ...
    ggsave(here("Figures", "07_analysis-comprehensive-coefs.png"), width = 5)
