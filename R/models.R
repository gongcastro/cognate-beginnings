...
get_vars_dict <- function(responses) 
{
    c(
        ...
        "b_n_phon_std" = glue::glue("Length (+1 SD, {round(sd(responses$n_phon), 2)} phonemes)"),
        "b_exposure_std" = glue::glue("Exposure (+1 SD, {round(sd(responses$exposure), 2)})"),
                                  ...
                                  ...) 
{
    ...
    # posterior summary
    posterior_summary <- bayestestR::describe_posterior(
        model,
        test = "rope",
...
get_posterior_draws_re <- function(model)
{
    ...
    posterior_draws <- gather_draws(model, `b_.*`, regex = TRUE) |>
        ...
        mutate(
            ...
            .variable = stringr::str_remove_all(.variable, "b_|\\[1\\]|\\[2\\]")
        )
