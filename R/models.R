...
#' @param model A brmsfit object
#' @param data Dataset of responses as generated by the [get_responses()].
#' @param ... Arguments to be passed to [tidybayes::posterior_predict()].
get_model_ppc <- function(model, data, ...)
{
    yrep_char <- posterior_predict(model, ...)
    ppcs <- sapply(data.frame(yrep_char, stringsAsFactors = TRUE),  as.integer)
