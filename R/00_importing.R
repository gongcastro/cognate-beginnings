...
get_bvq <- function(...){
    ...
    edu_levels <- c("noeducation", "primary", "secondary", "complementary", "vocational", "university") 
    edu_labels <- c("No education", "Primary", "Secondary", "Complementary", "Vocational", "University")
    
    l <- bvq_logs(p, r) %>% 
        mutate(
            across(starts_with("edu_"), ~as.numeric(factor(.x, levels = edu_levels, ordered = TRUE))),
    ...
    bvq_data <- list(participants = p, responses = r, logs = l, pool = pool)
