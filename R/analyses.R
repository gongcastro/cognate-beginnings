...
dat_responses <- responses %>% 
    ...
    mutate(response = factor(response, ordered = TRUE, labels = c("No", "Understands", "Understands & Says"))) %>% 
    arrange(item, te) %>% 
...
priors <- c(
    ...
    prior(normal(0.1, 0.15), class = b, coef = "age_centred"),
    prior(normal(0, 0.5), class = b, coef = "item_dominance1"),
    prior(normal(0, 0.5), class = b, coef = "age_centred:item_dominance1"),
    prior(normal(0, 0.5), class = b, coef = "cognateness1"),
    prior(normal(0, 0.5), class = b, coef = "age_centred:cognateness1"),
    prior(normal(0, 0.5), class = b, coef = "item_dominance1:cognateness1"),
    prior(normal(0, 0.5), class = b, coef = "age_centred:item_dominance1:cognateness1")
)
...
#### posterior -----------------------------------------------------------------
post <- gather_draws(fit2, `b.*`, regex = TRUE) %>% 
...
ggplot(post, aes(.value, fill = category)) +
