...
dat_participants <- fread(here("Data", "01_participants.csv"), na.strings = "")
...
# import results
comp_fit0 <- readRDS(here("Results", "comp_fit0.rds"))
comp_fit1 <- readRDS(here("Results", "comp_fit1.rds"))
comp_fit2 <- readRDS(here("Results", "comp_fit2.rds"))
comp_selection <- readRDS(here("Results", "comp_loo.rds"))

random <- readRDS(here("Results", "ranef.rds"))
corr <- readRDS(here("Results", "corr.rds"))
...
ggplot(posterior, aes(.value, .variable)) +
    ...
    stat_interval(.width = c(0.95, 0.89, 0.67, 0.5),
                  point_interval = "median_hdi",
                  position = position_nudge(y = -0.1),) +
    geom_vline(xintercept = 0, linetype = "dashed") +
    labs(x = "Value", y = "Coefficient", colour = "HDI", fill = "Type") +
    scale_colour_brewer() +
    theme_classic() +
...
posterior_check <- expand_grid(n = 1,
    ...
    add_fitted_draws(model = comp_fit2, newdata = ., n = 100, scale = "response", re_formula = NA) %>% 
    ungroup() %>% 
...
ggplot(posterior_check, aes(age_bin, .value, colour = cognate, fill = cognate)) +
    facet_grid(~item_dominance) +
    stat_lineribbon(.width = 89, colour = NA, alpha = 0.5) +
...
```{r random_effects}
ggplot(random, aes(r_intercept, item_dominance1)) +
    geom_point(shape = 1, alpha = 0.5, size = 0.25) +
    geom_abline(slope = corr$cor_meaning_mid_intercept_mid_item_dominance1, colour = "red") +
    labs(x = "Intercept", y = "Dominance") +
    
    ggplot(random, aes(r_intercept, cognate1)) +
    geom_point(shape = 1, alpha = 0.5, size = 0.25) +
    geom_abline(slope = corr$cor_meaning_mid_intercept_mid_cognate1, colour = "red") +
    labs(x = "Intercept", y = "Cognateness slope") +
    
    ggplot(random, aes(r_intercept, item_dominance1_cognate1)) +
    geom_point(shape = 1, alpha = 0.5, size = 0.25) +
    geom_abline(slope = corr$cor_meaning_mid_intercept_mid_item_dominance1_cognate1, colour = "red") +
    labs(x = "Intercept", y = "Dominance x Cognateness") +
    
    ggplot(random, aes(item_dominance1, cognate1)) +
    geom_point(shape = 1, alpha = 0.5, size = 0.25) +
    geom_abline(slope = corr$sd_meaning_mid_item_dominance1_cognate1, colour = "red") +
    labs(x = "Dominance", y = "Cognateness") +
    
    ggplot(random, aes(item_dominance1, item_dominance1_cognate1)) +
    geom_point(shape = 1, alpha = 0.5, size = 0.25) +
    geom_abline(slope = corr$sd_meaning_mid_item_dominance1_cognate1, colour = "red") +
    labs(x = "Dominance", y = "Dominance x Cognateness") +
    
    ggplot(random, aes(cognate1, item_dominance1_cognate1)) +
    geom_point(shape = 1, alpha = 0.5, size = 0.25) +
    geom_abline(slope = corr$sd_meaning_mid_item_dominance1_cognate1, colour = "red") +
    labs(x = "Cognateness", y = "Dominance x Cognateness") +
    
    plot_layout(ncol = 3) &
    plot_annotation(tag_levels = "A") &
    theme_classic() &
    ggsave(here("Figures", "random_effects.png"), height = 4, width = 6)
